...
if __name__ == "__main__":
    ...
    plt.subplot(131)
    plt.imshow(area1); plt.colorbar()
    plt.title('UCA Calculated on Full Tile')
    plt.subplot(132)
    plt.imshow(area2 - area1); plt.colorbar(label="all 0's expected")
    plt.title('UCA Calculation:\n(on chunk) - (on full tile)')
    plt.subplot(133)
    plt.imshow(area3 - area2); plt.colorbar(label="all 0's expected")
    plt.title('UCA Calculation:\n(on chunk, limited) - (on chunk)')
    plt.tight_layout()
    plt.show()
